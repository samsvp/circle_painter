 ...
 */
auto circle_img(const char* target_path, const char* background_path,
        const char* mask_path, float max_alpha, int f=2, int step=3)
{
    auto mask = cv::imread(mask_path);
    auto target = cv::imread(target_path);
    auto background = cv::imread(background_path);
    ...
    auto place_circles = [&img, &step, &down_background, &r, &sx, &iters, &max_alpha](
        const std::vector<std::vector<int>>& pixels_pos,
        const std::vector<cv::Vec3b>& target_pixels, int start, int end){
        // calculate background means for pixels inside the image
        for (size_t k = start; k < end; k++)
        {
            ...
            for (size_t i = 0; i < iters[k]; i++)
            {
                ...
                float alpha = (max_alpha * k) / r.size() + 0.05;
                img = nf::overlay(min_avg_img, img, min_avg_img != cv::Vec3b(0,0,0), alpha);
            }
    ...
    auto place_circles2 = [&img, &step, &down_target, &down_background, &r, &sx, &iters, &max_alpha](
        const std::vector<std::vector<int>>& pixels_pos,
        const std::vector<cv::Vec3b>& target_pixels, int start, int end) {
        // calculate background means for pixels inside the image
        for (size_t k = start; k < end; k++)
    ...
    f == 1 ? place_circles(pixels.first, pixels.second, 0, r.size() - 1) : 
        place_circles2(pixels.first, pixels.second, 0, r.size() - 1);
    ...
    f == 1 ? place_circles(pixels.first, pixels.second, r.size() - 1, r.size()) :
        place_circles2(pixels.first, pixels.second, r.size() - 1, r.size());
...
int main(int argc, char** argv)
{
    ...
    const int method = utils::parse_option("-f", 2, argc, argv);
    const float alpha = utils::parse_option("-a", 0.8f, argc, argv);

    auto img = circle_img(target_path, background_path, mask_path, alpha, method, 5);
